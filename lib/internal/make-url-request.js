...
module.exports = function makeUrlRequest(url, onSuccess, onError, options) {
  ...
  let log = debug && `google-maps-services-js - makeUrlRequest - url: ${url}`;
  if (proxy && proxy.proxyUrl !== proxyUrl) { // if we are being fed a new proxy, use it, otherwise store as global var so we dont create new proxy agent instances unecesessarily
    const proxyUrl = proxy.proxyUrl;
	const secureProxy = proxy.secureProxy;
	// details on using proxy auth: https://github.com/TooTallNate/node-https-proxy-agent/issues/12
    var proxyOpts = parse(proxyUrl);
    log += debug && ` | proxyUrl: ${proxyUrl} | secureProxy: ${secureProxy} | proxyOpts: ${JSON.stringify(proxyOpts)}`;
	proxyOpts.headers = proxyOpts.headers || {};
    proxyOpts.headers.keepAlive = true;
    if (secureProxy) {
      proxyOpts.secureProxy = true;
    }
